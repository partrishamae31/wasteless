...
import { 
  Package, MessageSquare, ArrowLeftRight, Plus, Bell, 
  User, Settings, Award, LogOut, Star, CheckCircle 
} from 'lucide-react';
...
<div className="relative">
  ...
  />
  <span className="absolute -top-1 -right-1 bg-red-500 text-white text-[10px] rounded-full w-4 h-4 flex items-center justify-center border-2 border-white">3</span>

  {/* Notifications Dropdown */}
  {showNotifications && (
    <div className="absolute top-10 right-0 w-80 bg-white rounded-2xl shadow-2xl border border-slate-100 z- overflow-hidden animate-in fade-in slide-in-from-top-2 duration-200">
      <div className="p-4 border-b border-slate-50 flex justify-between items-center">
        <h3 className="font-bold text-slate-800 text-sm">Notifications</h3>
        <div className="flex gap-3 items-center">
          <button className="text-[10px] text-[#3285a1] font-bold hover:underline">Mark all read</button>
          <Plus className="rotate-45 text-slate-400 cursor-pointer" size={16} onClick={() => setShowNotifications(false)} />
        </div>
      </div>
      ...
      <div className="max-h-[400px] overflow-y-auto">
        ...
        />
        <NotificationItem 
          icon={<ArrowLeftRight className="text-purple-500" size={14}/>}
          bg="bg-purple-50"
            ...
            <div className="bg-gradient-to-br from-emerald-600 to-teal-700 p-4 text-white">
              <div className="flex items-center gap-3 mb-3">
                ...
                <div className="overflow-hidden">
                  <div className="text-sm font-bold truncate">{session.user.email.split('@')}</div>
                  <div className="text-[10px] opacity-80 truncate">{session.user.email}</div>
                </div>
              </div>
              <div className="flex gap-2">
                <div className="flex-1 bg-white/10 rounded-lg p-1.5 text-center">
                  <div className="text-[10px] opacity-70 flex items-center justify-center gap-1"><Star size={10}/> Rating</div>
                  <div className="text-xs font-bold">0.0</div>
                </div>
                <div className="flex-1 bg-white/10 rounded-lg p-1.5 text-center">
                  <div className="text-[10px] opacity-70 flex items-center justify-center gap-1"><Package size={10}/> Listings</div>
        ...
        {['listings', 'messages', 'transactions'].map((tab) => (
          <button 
            ...
            className={`flex-1 flex items-center justify-center gap-2 py-3 rounded-lg text-sm font-medium transition capitalize ${
              activeTab === tab ? 'bg-[#3285a1] text-white' : 'text-slate-500 hover:bg-slate-50'
            }`}
          >
      ...
      <div className="grid grid-cols-3 gap-6">
        ...
        <div className="col-span-2">
          <div className="flex justify-between items-center mb-4">
            ...
            <button className="bg-[#3285a1] text-white px-4 py-2 rounded-lg text-sm font-medium flex items-center gap-2 hover:opacity-90 transition">
          ...
          <div className="space-y-4">
            {loading ? (
              <p className="text-center text-slate-400 py-10">Syncing with Wasteless database...</p>
            ) : listings.length > 0 ? (
              listings.map((item) => (
                <div key={item.id} className="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm relative overflow-hidden">
                  <div className="flex justify-between items-start mb-2">
                    <div>
                      <div className="flex items-center gap-2 mb-1">
                        <span className="font-bold text-slate-800">{item.device_model}</span>
                        <span className="bg-blue-100 text-blue-600 text-[10px] px-2 py-0.5 rounded-full font-medium uppercase">{item.status}</span>
                      </div>
                      <div className="text-xs text-slate-400 mb-2">{item.condition}</div>
                    </div>
                  ...
                  <div className="flex justify-between items-center border-t border-slate-50 pt-4 mt-4">
                    <div className="flex gap-4 text-xs font-bold text-[#3285a1]">
                      <span>₱ {item.scrap_value}</span>
                      <span className="flex items-center gap-1"><MessageSquare size={12}/> 0 bids</span>
                    </div>
                    <div className="text-[10px] text-emerald-500 flex items-center gap-1">
                      <span className="w-3 h-3 bg-emerald-100 rounded-full flex items-center justify-center">✓</span> Data cleared
                    </div>
                  </div>
                </div>
              ))
              ...
              <div className="bg-white p-10 rounded-2xl text-center border-2 border-dashed border-slate-200">
                <p className="text-slate-400">No active listings found in the database.</p>
              </div>
            )}
          </div>
        </div>
        ...
        {/* Right Panel */}
        <div className="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm flex flex-col items-center justify-center text-center h-fit py-20">
          ...
          <p className="text-xs text-slate-400 px-4">Select a listing to view active bids or start a conversation</p>
        </div>
      </div>
    ...
    <div className="bg-white w-full max-w-lg rounded-[32px] overflow-hidden shadow-2xl animate-in zoom-in duration-300">
      ...
      <div className="bg-gradient-to-br from-[#3285a1] to-emerald-500 p-8 text-white relative">
        ...
        <div className="flex items-center gap-4 mb-4">
          ...
          <div>
            <h2 className="text-xl font-bold">{profileData?.full_name || session.user.email.split('@')}</h2>
      ...
      <div className="p-8">
        ...
        <div className="grid grid-cols-4 gap-3 mb-8">
          {[
            { label: 'Total Listings', val: listings.length, color: 'bg-emerald-50' },
            { label: 'Items Sold', val: '0', color: 'bg-blue-50' },
            { label: 'Rating', val: '0.0', color: 'bg-emerald-50' },
            { label: 'Reviews', val: '0', color: 'bg-slate-50' }
          ].map((s, i) => (
            <div key={i} className={`${s.color} p-4 rounded-2xl text-center border border-white`}>
        ...
        <div className="space-y-6">
          ...
          <div className="grid gap-4">
            <InfoItem icon={<User size={16}/>} label="Full Name" val={profileData?.full_name || "Not set"} />
            <InfoItem icon={<Bell size={16}/>} label="Email Address" val={session.user.email} />
            <InfoItem icon={<Plus size={16}/>} label="Phone Number" val={profileData?.phone || "+63 9xx xxx xxxx"} />
            <InfoItem icon={<Plus size={16}/>} label="Barangay" val={profileData?.barangay || "Barangay Karuhatan"} />
          </div>
        </div>
...
const NotificationItem = ({ icon, bg, title, desc, time, unread }) => (
  <div className={`p-4 flex gap-3 hover:bg-slate-50 transition cursor-pointer relative ${unread ? 'bg-slate-50/50' : ''}`}>
    ...
    <div className="flex-1">
      ...
      <div className="flex justify-between items-center mt-1">
        ...
        <button className="text-[9px] text-slate-300 hover:text-red-400">Delete</button>
      </div>
    </div>
  </div>
);
