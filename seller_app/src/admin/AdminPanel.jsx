...
const AdminPanel = ({ onLogout }) => {
  ...
  const menuItems = [
    { id: "analytics", label: "Platform Analytics", icon: <LayoutDashboard size={18} /> },
    { id: "hotspots", label: "E-Waste Hotspots", icon: <MapPin size={18} /> },
    { id: "donation", label: "Donation Management", icon: <HeartHandshake size={18} /> },
    { id: "donated", label: "Donated Items Database", icon: <Database size={18} /> },
    { id: "dropoff", label: "Drop-off Points", icon: <Navigation size={18} /> },
    { id: "user-management", label: "User Management", icon: <Users size={18} />, badge: 3 },
    { id: "transaction", label: "Transaction Review", icon: <FileSearch size={18} />, badge: 2 },
    { id: "database", label: "Device Database", icon: <Database size={18} /> },
    { id: "valuation", label: "Valuation Model", icon: <BarChart3 size={18} /> },
    { id: "trust", label: "Trust Tiers", icon: <ShieldCheck size={18} /> },
    ...
    <div className="flex h-screen bg-[#F8FAFC] font-sans text-slate-900">
      ...
      <aside className="w-64 bg-[#1E293B] text-slate-400 flex flex-col shadow-xl">
        <div className="p-6 flex items-center gap-3 border-b border-slate-800/50">
          ...
          <div>
            <span className="font-bold text-white text-lg block leading-none">Wasteless</span>
            <span className="text-[10px] text-slate-500 uppercase tracking-tighter">Admin Panel</span>
          </div>
        </div>
          ...
          {menuItems.map((item) => (
            ...
            >
              <div className="flex items-center gap-3">
                <span className={activeTab === item.id ? "text-teal-400" : "text-slate-500 group-hover:text-slate-300"}>
                  {item.icon}
                </span>
      ...
      <main className="flex-1 overflow-y-auto bg-[#F8FAFC]">
        ...
        {!["analytics", "hotspots", "user-management"].includes(activeTab) && (
